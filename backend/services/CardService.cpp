...
void CardService::doDeposit(const std::string& cardNum, int amount)
{
    try
    {
        ...
        //_txRepo.save(cardNum, "", amount, TransactionType::DEPOSIT, TransactionStatus::SUCCESSFUL);
    }
    catch (const std::exception& e)
    {
        //_txRepo.save(cardNum, "", amount, TransactionType::DEPOSIT, TransactionStatus::FAILED);
        throw;
	}
}
...
void CardService::doWithdraw(const std::string& cardNum, int amount)
{
    ...
    {   
        ...
        //_txRepo.save(cardNum, "", amount, TransactionType::WITHDRAWAL, TransactionStatus::SUCCESSFUL);
    }
    catch (const std::exception& e)
    {
        //_txRepo.save(cardNum, "", amount, TransactionType::WITHDRAWAL, TransactionStatus::FAILED);
        throw;
    }
}
...
void CardService::doTransfer(const std::string& from, const std::string& to, int amount)
{
    try
    {
        ...
        //_txRepo.save(from, to, amount, TransactionType::TRANSFER, TransactionStatus::SUCCESSFUL);
    }
    catch (const std::exception& e)
    {
        //_txRepo.save(from, to, amount, TransactionType::TRANSFER, TransactionStatus::FAILED);
        throw;
    }
}
